...
while True:
    ...
    requestList = request.decode('utf-8').split()
    ...
    if requestList[0] == 'GET':
        ...
        if(requestList[1]=='/'):
            ...
            http_response = http_response + file.read()
            file.close()
            client_connection.send(http_response.encode('utf-8'))

        else:
            try:
                file = open(requestList[1][1:])
            except:
                http_response= """HTTP/1.1 404 NOT FOUND\r\n\r\n"""
                file = open('./server/notFound.html', 'r')
        ...
        file = open('./server/invalid.html', 'r')
        http_response = http_response + file.read()
        file.close()
